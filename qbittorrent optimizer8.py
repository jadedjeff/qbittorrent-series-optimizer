...
def any_torrents_active(qb):
    """Return True if any torrent is still downloading or fetching metadata."""
    active_states = {"downloading", "stalledDL", "metaDL", "checkingDL", "allocating"}

    torrents = qb.torrents_info()
    for torrent in torrents:
        # If torrent is in an active state → still active
        if torrent.state in active_states:
            return True

        # Otherwise, check if any files are incomplete
...
def timed_input(prompt, timeout=15):
    """Prompt user for input with a timeout. Returns None if timed out."""
    result = [None]
...
def manage_priorities():
    ...
    while True:
        ...
        for torrent in torrents:
            ...
            # 🚨 Skip torrents that are still fetching metadata
        ...
        # Check if all torrents are complete
        if not any_torrents_active(qb):
            logging.info("All downloads completed.")
            user_input = timed_input(
                "Press 'n' within 15 seconds to continue monitoring, or anything else to exit: ", 15
            )
            if user_input != 'n':
