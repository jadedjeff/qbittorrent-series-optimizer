...
def check_pip_update():
    ...
    try:
        ...
        # Get latest pip version from PyPI
        result = subprocess.run([sys.executable, "-m", "pip", "index", "versions", "pip"],
                                capture_output=True, text=True)
        latest_line = next((line for line in result.stdout.splitlines() if "Available versions" in line), "")
        if latest_line:
            latest_version_str = latest_line.split(":")[-1].split(",")[0].strip()
